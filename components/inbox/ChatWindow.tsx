            ...
            {isAutomationOpen && (
                <div className="absolute bottom-20 left-4 right-4 md:left-auto md:right-4 md:w-80 bg-black/90 border border-white/10 rounded-xl shadow-2xl backdrop-blur-xl p-2 z-50">
                    ...
                    <div className="max-h-60 overflow-y-auto space-y-1">
                        {/* yalnizca duz-metin cevapli kurallar: kart kurallarinda .message yok,
                            eskiden kuralin ADI musteriye DM olarak gidiyordu */}
                        {automations.filter(a => a.response_content?.message).map(auto => (
                            <button
                                key={auto.id}
                                onClick={() => handleSendMessage(auto.response_content.message)}
